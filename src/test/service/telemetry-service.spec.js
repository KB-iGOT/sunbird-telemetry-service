...
describe('telemetry Service', () => {
    ...
    describe('Message Size Handling Tests', () => {
        const fs = require('fs');
        const path = require('path');
        const { generateLargeEvent } = require('../data/generate-large-event');
        ...
        beforeEach(() => {
            ...
            process.env.telemetry_log_level = 'info';
            process.env.telemetry_local_storage_type = 'kafka';
            req.body = JSON.parse(fs.readFileSync(path.join(__dirname, '../data/normal-event.json'), 'utf8'));
        });

        it('should handle normal size message successfully', function () {
            telemetryService = require(telemetryServicePath);
            sinon.stub(telemetryService.dispatcher, 'dispatch').callsFake((message, body, cb) => cb(null, {body: {}}));
        ...
        it('should handle large message that causes MessageSizeTooLarge error', function () {
            req.body = generateLargeEvent(1); // 1MB event
            telemetryService = require(telemetryServicePath);
            
            // Simulate Kafka MessageSizeTooLarge error
            const kafkaError = new Error('MessageSizeTooLarge');
            kafkaError.name = 'MessageSizeTooLarge';
            sinon.stub(telemetryService.dispatcher, 'dispatch').callsFake((message, body, cb) => cb(kafkaError));
            ...
            const errorCall = res.json.getCall(0);
            expect(errorCall.args[0]).to.have.property('responseCode', 'SERVER_ERROR');
            expect(errorCall.args[0]).to.have.property('params');
            expect(errorCall.args[0].params).to.have.property('err');
        });

        it('should log complete event details when MessageSizeTooLarge error occurs', function () {
            const consoleSpy = sinon.spy(console, 'log');
            req.body = generateLargeEvent(1);
            telemetryService = require(telemetryServicePath);
            ...
            // Check that error and event details are logged
            expect(consoleSpy.calledWith('error')).to.be.true;
            expect(consoleSpy.calledWith('Complete event details:')).to.be.true;
            
            consoleSpy.restore();
        });

        it('should log error response when sendError is called', function () {
            const consoleSpy = sinon.spy(console, 'log');
            telemetryService = require(telemetryServicePath);
            
            const kafkaError = new Error('Test error for logging');
            sinon.stub(telemetryService.dispatcher, 'dispatch').callsFake((message, body, cb) => cb(kafkaError));
            
            telemetryService.dispatch(req, res);
            
            // Check that error response is logged
            expect(consoleSpy.calledWith('Error response:')).to.be.true;
            
            consoleSpy.restore();
        });

        it('should handle event with large edata field', function () {
            req.body = {
                eid: 'TEST',
                edata: {
                    largeField: 'x'.repeat(100000), // 100KB string
                    description: 'Test event with large edata field'
                }
            };
            
            telemetryService = require(telemetryServicePath);
            sinon.stub(telemetryService.dispatcher, 'dispatch').callsFake((message, body, cb) => {
        ...
        it('should preserve message mid when handling large messages', function () {
            const testMid = 'test-mid-12345';
            req.body = {
                mid: testMid,
                eid: 'TEST',
                edata: { test: 'data' }
            ...
            sinon.stub(telemetryService.dispatcher, 'dispatch').callsFake((message, body, cb) => {
                ...
                expect(parsedMessage.mid).to.equal(testMid);
                cb(null, {body: {}});
            });
